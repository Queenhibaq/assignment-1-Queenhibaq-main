...
export class App extends gfx.GfxApp {
  ...
  createScene(): void {
    ...
    for (let i = 0; i < this.starfield.numParticles; i++) {
      ...
      this.starfield.particlePositions[i].set(
        Math.random() * 2 - 1,
        Math.random() * 2 - 1
      );
    }
    ...
    this.laser.material.color.set(0.247, 0.995, 0.284, 1);
    ...
    // these values.
    this.mine.boundingCircle.radius *= 0.75;
  ...
  update(deltaTime: number): void {
    ...
    for (let i = 0; i < this.starfield.numParticles; i++) {
      ...
      const startrack = gfx.Vector2.multiplyScalar(
        moveDirection,
        shipSpeed * starSize * 200
      );
      starPosition.subtract(startrack);
      ...
      if (starPosition.x > 1) starPosition.x += -2;
      else if (starPosition.x < -1) starPosition.x += 2;

      if (starPosition.y > 1) starPosition.y += -2;
      else if (starPosition.y < -1) starPosition.y += 2;
    }
    ...
    this.mines.children.forEach((mine: gfx.Node2) => {
      ...
      const mineToShip = gfx.Vector2.subtract(
        mine.position,
        this.ship.position
      );
      ...
      const mineShift = gfx.Vector2.multiplyScalar(moveDirection, shipSpeed);
      mine.position.subtract(mineToShip);
    ...
    this.lasers.children.forEach((laser: gfx.Node2) => {
      // ADD YOUR CODE HERE
      const laserShift = new gfx.Vector2(0, laserSpeed);
      laserShift.rotate(laser.rotation);
      laser.position.add(laserShift);
      if (
        laser.position.x > 1 ||
        laser.position.x < -1 ||
        laser.position.y > 1 ||
  ...
  onMouseDown(event: MouseEvent): void {
    const laserShot = gfx.Geometry2Factory.createBox();
    laserShot.material.color.set(0.561, 0.184, 0.561, 1);
    laserShot.scale.x = 0.01;
    laserShot.scale.y = 0.1;
    const laserDirection = laserShot.lookAt(this.mousePosition);
    this.laser.add(laserShot);
    // ADD YOUR CODE HERE
  ...
  onMouseMove(event: MouseEvent): void {
    this.mousePosition.copy(
      this.getNormalizedDeviceCoordinates(event.x, event.y)
    );
  }
  ...
  private spawnMine(): void {
    ...
    const mineRotation =
      this.ship.rotation + ((Math.random() * Math.PI) / 3 - Math.PI / 6);
    const mineDirection = gfx.Vector2.rotate(gfx.Vector2.UP, mineRotation);
    mineInstance.position.add(
      gfx.Vector2.multiplyScalar(mineDirection, mineSpawnDistance)
    );
    ...
    if (this.mines.children.length > mineLimit) {
      ...
      if (!firstMine.isExploding()) firstMine.explode();
    }
  }
    ...
    this.lasers.children.forEach((laser: gfx.Node2) => {
      this.mines.children.forEach((mineElem: gfx.Node2) => {
        ...
        if (
          laser.intersects(
            mine,
            gfx.IntersectionMode2.AXIS_ALIGNED_BOUNDING_BOX
          )
        )
          mine.explode();
        laser.remove();
